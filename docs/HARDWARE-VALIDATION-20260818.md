...
WARN 对应当时未接显示器、第二网口无载波和没有摄像头。NPU 已由脚本自动选择实际
存在的 MobileNet 模型并成功推理。后续插入的 UVC 摄像头测试见下表和
`HARDWARE-DISCOVERY-20260818.md`。
...
| J2600 刷机 Type-C | `fc000000.usb` UDC；原厂 DT 还定义 HUSB311 双角色 PD 与 DP Alt Mode | 刷机实测；PD/DRD/DP 为硬件强证据、当前固件未启用 |
...
| J9400 | CAN/UART/电源类机器人主线束候选 | 功能仍属推断，禁止按猜测接线 |
| J7002 | 2×4 未知接口，位于 `70xx` 音频区域 | 原“USB×2 主线束”证据不足，已撤回 |
| J9701 | 1×4 带 +12V 的差分总线候选；从上到下：1=2.42V、2=2.52V、3=GND/0V、4=+12V | 前三针与 J9702 逐针直连；CAN/RS-485 协议仍待波形确认 |
...
| J9702 | 1×3 差分总线候选；左→右 2.42V / 2.52V / GND，信号脚间 118.8Ω | 与 J9701 1/2/3 逐针直连；电气形态确认，协议未确认 |
| J9703 | 1×3 独立差分总线候选；左→右 2.49V / 2.49V / GND，信号脚间 120.7Ω | 电气形态确认；CAN/RS-485、极性及控制器映射待确认 |
...
6. 普通 UVC 相机已完成枚举、格式和 30 帧连续采集；仍需用原目标 Berxel/Orbbec 深度相机验证多机带宽、深度流和 ROS 驱动。
7. 核对 J9400/J7002 的原理图或用示波器逐针测量。J970x 应先完成断电通断/电阻测试，再用示波器或 CAN 分析仪确认 CANH/CANL、can0/can1 与真实波特率。

### J970x 差分总线区交叉分析（2026-08-18）

- 电气实测：J9701 前三针与 J9702 三针均为 `2.42 / 2.52 / 0V`；J9703 为 `2.49 / 2.49 / 0V`。两信号脚约 2.5V、第三脚接地符合 CAN 的静止态特征，但 RS-485 等差分物理层也可能采用类似共模与 120Ω 终端；J9701 额外提供 +12V。
- 断电实测：J9701 1/2/3 与 J9702 左/中/右分别为 `0Ω / 0Ω / 0.03Ω`，确认逐针并联；J9702 两信号脚间 `118.8Ω`，J9703 两信号脚间 `120.7Ω`，确认两组独立、带约 120Ω 终端的差分网络。
...
已确认拓扑仅限：J9701 与 J9702 是同一差分网络的带电/不带电两个出口，J9703 是另一组独立差分网络。尚未确认 CAN/RS-485 协议、差分极性、是否对应 can0/can1，以及原厂实际波特率。
...
- 因此不能仅凭“两路 CAN 控制器 + 两组差分座”就断言协议和物理映射。剩余可能包括 97xx 区域并非 CAN 收发器、收发器 standby/silent 使能未打开、物理座接到板载 MCU/不同总线域，或还存在未识别的收发路径。下一步应先识别 U970x 丝印或做受控发送波形定位，而不是继续盲目交换线序。
