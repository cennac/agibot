...
- `IMG_20260812_114650.jpg` 确认主电源区域的器件顺序为 J7001、J7000、J9703、J9702、J2000。J9703/J9702 是两个上下相邻的 `1×3` 排针，组合外观为 `2×3`；紧靠 J7000 白色座的上排为 J9703，下排为 J9702。2026-08-18 电压、通断和终端电阻实测结合原厂 DT、机器人 `canclient` 和 97xx 收发器电路区，已确认两座为两路 CAN
...
### CAN 总线（2 路，机器人底盘/电机总线）

`flexcan fea50000 / fea60000` → **can0 / can1**。scrubber 经 CAN 与下位机通信，属核心端口，上文 USB 实测未涉及。
...
| CAN | **2 口**（can0 / can1） | 实机 |
| WiFi | Broadcom 449d（wlan0 / wlan1） | lspci |
| SATA | 3 节点全 disabled，未用 | dts + 实机 |
| 显示 | HDMI0 / HDMI1 + edp + dp | dts |
| UART | 10 路（serial0 debug + 1–9） | dts |

### 仍需原理图 / 实测确认（软件枚举够不着）

`J2600`、`J9304`、`J2000` 三针主电源，以及扬声器 `J7000 / J7001` 的左右声道与正负针序仍需原理图或电气实测。J9702/J9703 已由电压、通断、约 120Ω 终端、DT、程序和照片共同确认为两路 CAN，但 CANH/CANL 顺序和 can0/can1 对应关系仍未实证。J8900/J8901 已由丝印确认：从上到下分别为 `SWDIO/SWCLK/GND` 与 `TX/RX/GND`。2026-08-18 J8901 短接回环进一步确认其为 `UART0 / ttyS0`（TX=`GPIO0_C5`，RX=`GPIO0_C4`）；测试时还收到 `the vol_12v is :11.74V`，说明线路可能并接电压监测控制器，不能长期短接。
...
| J9400 | 2×5=10 针,右缘主电源上方 | **机器人主线束**:CAN0/CAN1 + UART×1~2 + 电源检测/GND(通往 SCU 下位机) | 高 |
| J7002 | 2×4=8 针,同区 | **主线束第二段**:USB×2 对(机体相机回传)+ 5V + GND,或 2×UART+电源 | 高 |
| J9701 | 1×4 白座,右上 | **带 +12V 的 CAN-A 接口**；从上到下为差分对 / GND / +12V，前三针与 J9702 逐针直连 | 功能、供电、并联关系已实测；CANH/CANL 顺序待确认 |
| J9303 | 2 针,双网口与 HDMI 之间 | 蜂鸣器 或 电源指示/开机键(音频区只有喇叭,清洁机需警示音) | 中 |
| J5001 | 2 针,USB3000 与「4G」J9200 之间 | 4G 模块控制线(PWRKEY/RESET)或 VBUS 使能 | 中 |
| J9702 | 1×3,喇叭/电源区 | **CAN-A**；左→右 2.42V / 2.52V / GND，与 J9701 前三针逐针直连，差分对间 118.8Ω | 功能/终端/并联关系已实测；极性/控制器映射待确认 |
| J9703 | 1×3,紧靠 J7000 | **CAN-B**；左→右 2.49V / 2.49V / GND，差分对间 120.7Ω | 功能/终端已实测；极性/控制器映射待确认 |
...
- **J970x 已确认是双路 CAN 接口区**：原厂 DT 仅启用 can0/can1，当前 Armbian 映射为 `can0=fea50000`、`can1=fea60000`；机器人 ROS 消息明确存在 `canclient` 到四块 SCU/MCU。断电通断确认 J9701 前三针与 J9702 逐针并联；J9702/J9703 的差分对分别为 118.8Ω/120.7Ω
...
5. **J9701**:必须完全断电后测 1–2 脚电阻；约 60Ω 或 120Ω 可进一步支持 CAN 判断。上电时用示波器或 CAN 分析仪判定 CANH/CANL，严禁短接 3 脚 GND 与 4 脚 +12V
