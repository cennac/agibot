...
2026-08-22 LEDE round10 复测修正：仅把 RK806 `SYS_CFG3[7:6]` 写为 `01` 并读回
`SYS_CFG3=0x42` 仍不足以让 SW9201 复位；系统运行态与启动早期轻按均无效。
`rk805_pwrkey` input 已注册，但 `rk805_pwrkey_fall/rise` IRQ 计数保持 0，
button-hotplug 日志也为空。因此 SW9201 依赖 RK806 及其完整 vendor 初始化或
另一条板级路径，不能继续简化为单个 `pmic-reset-func` 写值，也不能把它和
SW8900 的发行版无关硬复位网混为一谈。
